...
| `/play`   | включить трек: название или ссылка (YouTube/Spotify/Яндекс/SoundCloud) |
| `/skip` `/stop` `/pause` `/resume` `/repeat` `/queue` | управление музыкой |
| `/joke`   | пошутить прямо сейчас                                   |
| `@бот ...` | (в текстовом чате) ответит на сообщение                |

Музыку можно ставить тремя способами: командой `/play`, голосом («друг, включи …»)
и просто кинув ссылку в текстовый канал, где бота позвали. Под каждым найденным треком
висит список других вариантов — поиск иногда берёт кавер или не ту версию; голосом то же
самое делает фраза «не то, давай другой вариант».

Трек можно не называть, а описать: «та песня из Аркейна, где Экко и Джинкс, голосом
Грагаса». На такие просьбы (и на любые, где буквальный поиск дал непохожее) подключается
модель: она предлагает свой поисковый запрос и выбирает нужное из того, что реально
нашлось на YouTube. На обычные названия модель не зовётся — поиск и так справляется.

## Лимиты Groq

На free-тарифе у каждой модели свой суточный лимит токенов (у `gpt-oss-120b` — 200k).
Один ответ в войсе стоит около 1500 токенов, и 97% из них — вход: системный промпт с
характером, меню мем-звуков и последние реплики отправляются заново на КАЖДУЮ фразу.
То есть лимита хватает примерно на 130-140 ответов — на один вечер.

Что с этим сделано:

- исчерпанная модель откладывается ровно на то время, которое называет Groq в заголовке
  `retry-after`, и бот доживает день на запасной (`gpt-oss-20b`, затем `qwen3.8-27b` — у
  каждой свой лимит). В чат при этом падает предупреждение, что мозг сменился;
- меню мем-звуков (240 токенов) не отправляется, пока звук на кулдауне и всё равно не
  прозвучит;
- служебные запросы поиска музыки идут в младшую модель (`SEARCH_MODEL`), чтобы не
  отъедать бюджет разговора;
- расход за сегодня виден командой `/tokens` и на странице статуса сервиса.

Если и этого мало — режьте `RECENT_TURNS` (история на каждый запрос) и `BANTER_RULES`
в `services/llm.py`, они дают почти весь вход, либо переходите на платный тариф.
